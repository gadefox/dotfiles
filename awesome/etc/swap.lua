...
local function bydirection(dir, swap)
  ...
  if swap then
    ...
    gears.timer.delayed_call(function() display(cur, swap) end)
  else
    client.focus = target
...
local function create_arc(idx, head, tail)
  ...
  return wibox.widget({
    shape = function(cr, width, height) gears.shape.arc(cr, width, height, dpi(12), math.pi * idx / 2, math.pi * (idx + 1) / 2, head, tail) end,
    bg = palette[idx],
...
local function create_arcs()
  for i = 0, 4 do
    local arc = wibox({ width = 100, height = 100, ontop = true })
    local stack = { layout = wibox.layout.stack }

    if i == 4 then
      arc.shape = function(cr, width, height) gears.shape.arc(cr, width, height, dpi(12), 0, math.pi * 2, false, false) end

      for k = 0, 3 do
        table.insert(stack, create_arc(k, false, false))
      end
    else
      arc.shape = function(cr, width, height) gears.shape.arc(cr, width, height, dpi(12), math.pi * i / 2, math.pi * (i / 2 + 1), true, true) end
