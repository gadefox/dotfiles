...
#
# /neatnick set [<network>/<#channel>] <nick> <colour>
# * set a fixed colour for nick
#
# /neatnick reset [<network>/<#channel>] <nick>
# * remove a set colour of nick
#
# /neatnick get [<network>/<#channel>] <nick>
# * query the current or set colour of nick
#
# /neatnick re [<network>/<#channel>] <nick>
...
#
# /neatnick colors add <list of colours>
# /neatnick colors remove <list of colours>
...
sub _colours_set {
    my $scolours = shift;
    Irssi::settings_set_str('neatnick_colors', join '', map { $ext_to_base_map{$_} // "X$_" } grep { exists $scolours->{$_} } @colour_list);
}
...
sub neatnick_colors_add {
    my ($data, $server, $witem) = @_;
    my ($added, $scolours) = _colours_check(1, $data);
    if (@$added) {
      _colours_set($scolours);
      Irssi::print("%b %|NeatNick:%n Added @{[ _colours_list($added) ]} color(s)", MSGLEVEL_CLIENTCRAP);
      setup_changed();
    }
    else {
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_not_added');
    }
}

sub neatnick_colors_remove {
    my ($data, $server, $witem) = @_;
    my ($removed, $scolours) = _colours_check(0, $data);
    if (@$removed) {
      _colours_set($scolours);
      Irssi::print("%b %|NeatNick:%n Removed @{[ _colours_list($removed) ]} color(s)", MSGLEVEL_CLIENTCRAP);
      setup_changed();
    }
    else {
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_not_removed');
    }
}
...
sub neatnick_colors {
    ...
    $data =~ s/\s+$//;
    unless (length $data) {
      Irssi::print("%b %|NeatNick:%n Current colors: @{[ @colours ? _colours_list(\@colours) : '(none)' ]}", MSGLEVEL_CLIENTCRAP);
    }
    Irssi::command_runsub('neatnick colors', $data, $server, $witem);
}
...
sub neatnick {
    ...
    unless (length $data) {
      ...
      $witem->print('%b %|NeatNick: %nColor distribution: '.
        (join ', ',
        map { "%X$_$_:$distribution{$_}" }
        sort { $distribution{$b} <=> $distribution{$a} }
        grep { exists $distribution{$_} } @colour_list), MSGLEVEL_CLIENTCRAP);
    }
    Irssi::command_runsub('neatnick', $data, $server, $witem);
}

sub _check_netchan_arg {
    my ($cmd, $netchan, $nick) = @_;
    my %global = map { $_ => undef } qw(set get reset);
    unless (length $netchan) {
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_err_netchan_arg', $cmd, (exists $global{$cmd} ? ', use / to '.$cmd.' global colors' : ''));
    	return;
    }
    elsif (-1 == index $netchan, '/') {
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_err_no_net', $cmd);
      return;
    }
    elsif ($netchan =~ m\^[^/]+/$\) {
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_err_no_chan', $cmd);
      return;
    }

    unless (length $nick) {
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_err_no_nick', $cmd);
      return;
    }
    elsif (-1 != index $nick, '/') {
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_err_wrong_nick', $cmd);
      return;
    }
...
sub _check_colour {
    ...
    unless (length $colour && grep { $_ eq $colour } @colour_list) {
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_err_no_color', $cmd);
      return;
    }
    return $colour;
}
...
sub neatnick_set {
    ...
    my @args = split ' ', $data;
    if (@args < 2) {
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_err_arg_count', 'set');
      return;
    }
    my $netchan;
    if (ref $witem) {
    ...
    return unless _check_netchan_arg('set', $netchan, $nick);
    return unless defined ($colour = _check_colour('set', $colour));
    ...
    Irssi::print("%b %|NeatNick: %n%X$colour$nick%n colour set to: %X$colour$colour%n ".($netchan eq '/' ? 'globally' : "in $netchan"), MSGLEVEL_CLIENTCRAP);
}
...
sub neatnick_get {
    ...
    my @args = split ' ', $data;
    if (@args < 1) {
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_err_arg_count', 'get');
      return;
    }
    my $netchan;
    if (ref $witem) {
    ...
    return unless _check_netchan_arg('get', $netchan, $nick);

    if ($netchan ne '/') {
      unless (exists $has_colour{$netchan} && exists $has_colour{$netchan}{$nick}) {
        Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_no_color', $nick, $netchan);
    	}
      else {
        my $colour = $has_colour{$netchan}{$nick};
        Irssi::print("%b %|NeatNick: %n%X$colour$nick%n has color: %X$colour$colour%n in $netchan", MSGLEVEL_CLIENTCRAP);
      }
    }
    my $hashnick = $nick;
    $hashnick =~ s/$ignore_re//g if (defined $ignore_re && length $ignore_re);
    if (exists $set_colour{$netchan} && exists $set_colour{$netchan}{$nick}) {
        ...
        Irssi::print("%b %|NeatNick: %nColor for %X$colour$nick%n in $netchan is %X$colour$colour%n", MSGLEVEL_CLIENTCRAP);
    }
    elsif (exists $set_colour{$netchan} && exists $set_colour{$netchan}{$hashnick}) {
        my $colour = $set_colour{$netchan}{$hashnick};
        Irssi::print("%b %|NeatNick: %nColor for %X$colour$hashnick%n in $netchan is %X$colour$colour%n", MSGLEVEL_CLIENTCRAP);
    }
    elsif (exists $set_colour{''} && exists $set_colour{''}{$nick}) {
        my $colour = $set_colour{''}{$nick};
        Irssi::print("%b %|NeatNick: %nGlobal color for %X$colour$nick%n is %X$colour$colour%n", MSGLEVEL_CLIENTCRAP);
    }
    elsif (exists $set_colour{''} && exists $set_colour{''}{$hashnick}) {
        my $colour = $set_colour{''}{$hashnick};
        Irssi::print("%b %|NeatNick: %nGlobal color for %X$colour$hashnick%n is %X$colour$colour%n ", MSGLEVEL_CLIENTCRAP);
    }
    elsif ($netchan eq '/') {
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_no_global', $nick);
    }
}
...
sub neatnick_reset {
    ...
    my @args = split ' ', $data;
    if (@args < 1) {
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_err_arg_count', 'reset');
      return;
    }
    my $netchan;
    if (ref $witem) {
    ...
    if (@args == 1 && $args[0] eq '--all') {
      ...
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_reset');
      return;
    }
    if (@args < 2) {
      $nick = $args[0];
    ...
    return unless _check_netchan_arg('reset', $netchan, $nick);

    $netchan = '' if $netchan eq '/';
    unless (exists $set_colour{$netchan} && exists $set_colour{$netchan}{$nick}) {
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_no_color', $nick, (length $netchan ? "in $netchan" : "globally"));
      return;
    }
    ...
    Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_err_nick_reset', $nick, ($netchan eq '' ? '' : " in $netchan"), ($netchan eq '' ? '(global) ' : ''));
}
...
sub neatnick_re {
    ...
    my @args = split ' ', $data;
    if (@args < 1) {
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_err_arg_count', 're');
      return;
    }
    my $netchan;
    if (ref $witem) {
    ...
    return unless _check_netchan_arg('re', $netchan, $nick);

    unless (exists $has_colour{$netchan} && exists $has_colour{$netchan}{$nick}) {
      Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_not_found', $nick, $netchan);
      return;
    }
    ...
    Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_recolor', $nick, $netchan);
}

sub neatnick_save {
    Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_file_save');
    save_colours();
}

sub setup_changed {
    $global_colours = Irssi::settings_get_bool('neatnick_global');
    $retain_colour_time = int( abs( Irssi::settings_get_time('neatnick_reassign_time') ) / 1000 );
    my $old_ignore = $ignore_setting // '';
    $ignore_setting = Irssi::settings_get_str('neatnick_ignorechars');
    if ($old_ignore ne $ignore_setting) {
      local $@;
    	eval { $ignore_re = qr/$ignore_setting/ };
      if ($@) {
        $@ =~ /^(.*)/;
        Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_err_compile', $1);
    	}
    }
    my $old_colours = "@colours";
    my %scolours = map { ($base_map{$_} // $_) => undef } Irssi::settings_get_str('neatnick_colors') =~ /(?|x(..)|(.))/ig;
...
Irssi::command_bind({
    'help' => sub { &help_neat if $_[0] =~ /^neat\s*$/i;},
    'neatnick'               => 'neatnick',
    'neatnick save'          => 'neatnick_save',
    'neatnick set'           => 'neatnick_set',
    'neatnick get'           => 'neatnick_get',
    'neatnick reset'         => 'neatnick_reset',
    'neatnick re'            => 'neatnick_re',
    'neatnick colors'        => 'neatnick_colors',
    'neatnick colors add'    => 'neatnick_colors_add',
    'neatnick colors remove' => 'neatnick_colors_remove',
});

Irssi::signal_add_last('setup changed' => 'setup_changed');

Irssi::printformat(MSGLEVEL_CLIENTCRAP, 'neatnick_loaded', $VERSION);
